...
namespace SoldatovaCRUD.Pages
{
    ...
    public partial class order : Page
    {
        ...
        public class UserOrder
        {
            ...
            public string NameMerch { get; set; }
        ...
        public order(Order Item, int UsersID)
        {
            InitializeComponent();
            ...
            //dataGrid.ItemsSource = classes.connect.modelbd.Orders.Where(row => row.UserID == UsersID).ToList();
            List<UserOrder> items = new List<UserOrder>();
            ...
            if(userOrder != null && UserData != null && Place != null && MerchOrder != null)
            {
                items.Add(new UserOrder()
                {
                    Name = UserData.name,
                    Sale = userOrder.Sale,
                    Cost = userOrder.Cost,
                    NameMerch = MerchOrder.name,
                    Place = Place.Place1,
                    Arrived = userOrder.Arrived,
                    Code = userOrder.Code,
                    DateArrive = userOrder.DateArrive,
                    DateOrder = userOrder.DateOrder,
                    Picture = MerchOrder.picture
            ...
            {
                MessageBox.Show("1", UsersID.ToString(), MessageBoxButton.OK, MessageBoxImage.None);
            }

            MerchName.Text = $"Название товара: {MerchOrder.name}";
            MerchCost.Text = $"Цена: {userOrder.Cost}";
            MerchSale.Text = $"Скидка: {userOrder.Sale}%";
            string v = $"Дата заказа: {userOrder.DateOrder}".Trim();
            Date1.Text = v;
            Date2.Text = $"Дата приезда: {userOrder.DateArrive}".Trim();
            //CostText.Text = items.ToList().ToString();
            PlaceText.Text = $"Пункт выдачи: {Place.Place1}";
            CodeText.Text = $"Код:{userOrder.Code}";
        ...
        private void GenerateBarcodeButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                IDocumentPaginatorSource idp = token;
                pd.PrintDocument(idp.DocumentPaginator, "token 1");
            }
        }
